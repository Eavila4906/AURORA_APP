...
export class UsuariosComponent implements OnInit {
  ...
  getUser(id: number) {
    this.UsuariosService.get(id).subscribe(
      response => {
        ...
        this.userRoles = response.data.roles.map((rol: any) => `${rol.rol} - ${rol.app}`).join(', ');
  ...
  getRoles () {
    this.RolesService.getAll().subscribe( 
      response => {
        this.roles = response.data.filter( (rol: any) => rol.status === 1);
      }
    );
  }
  ...
  getEmpresas () {
    this.EmpresasService.getAll().subscribe( 
      response => {
        this.companies = response.data.filter( (company: any) => company.status === 1);
      }
    );
  }
