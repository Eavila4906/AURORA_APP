...
export class NuevaComponent implements OnInit {
  ...
  createFactura(op: number) {
    ...
    let data: DataStructureFactura = {
      data: {
        ...
        ordenTrabajo: {
          vehiculo_id: this.vehiculo_id ? this.vehiculo_id : null,
          fecha: this.fechaOrden,
          descripcion: this.descripcionOrden,
          mantenimientoVehiculo: this.productosEnFactura.map(producto => ({
            tipo: 'Mantenimiento',
            observaciones: producto.descripcion,
          })),
          generarTurno: this.generarTurno
        },
      ...
      response => {
        if (response.data) {
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              this.comprobante(data, response.data,);
            }
          })
        }
      }
  ...
  selectProducto(productoSeleccionado: any) {
    ...
    if (productoExistente) {
      ...
      const newProducto = {
        ...
        preciosVenta: [
          { pvpIva0: productoSeleccionado.pvp1Iva0 },
          { pvpIva0: productoSeleccionado.pvp2Iva0 },
          { pvpIva0: productoSeleccionado.pvp3Iva0 },
          { pvpIva0: productoSeleccionado.pvp4Iva0 },
        ].filter(p => p.pvpIva0 > 0),
        precioUnitario: productoSeleccionado.pvp1Iva0,
  ...
  selectOption(option: any) {
    ...
    this.searchClientes = '';
    this.clientesFilter = this.clientes;
    this.vehiculosClientes = option.vehiculo;
  }
  ...
  public comprobante(data: any, dataResp: any) {
    ...
    const docDefinition: any = {
      ...
      content: [
        '***************************',
        { text: `COMPROBANTE`, style: 'header', alignment: 'center' },
        { text: `${data.data.cabecera.perfil}`, style: 'titleEmpresa', alignment: 'center' },
        { text: `N° Orden: ${dataResp.orden.orden.codigoOrden}`, style: 'info', alignment: 'center' },
        '------------------------------------------',
        { text: `Turno: ${dataResp.orden.turno.turno}`, style: 'dataCliente' },
        { text: `Fecha: ${dataResp.orden.turno.fecha}`, style: 'dataCliente' },
        { text: `Factura: ${dataResp.cabecera.codigoFactura}`, style: 'dataCliente' },
