...
export class IngresosComponent implements OnInit {
  ...
  constructor(
    ...
    private IngresosService: MovimientosService,
  ...
  ngOnInit(): void {
    ...
    this.IngresosService.getAll().subscribe( 
      response => {
        this.ingresos = response.data.filter( (ingreso: any) => ingreso.movimiento_id === 1 )
        .sort((a: any, b: any) => b.id - a.id);
        this.ingresosFilter = this.ingresos;
        this.loading = false;
      }
    ); 
  ...
  getIngreso(id: number) {
    this.IngresosService.get(id).subscribe(
      response => {
  ...
  create() {
    ...
    this.IngresosService.create(data).subscribe(
      response => {
        if (response.data) {
  ...
  edit(id: number) {
    ...
    this.IngresosService.edit(data).subscribe(
      response => {
        if (response.data) {
          this.toastr.success(response.message, '¡Listo!', {closeButton: true});
          this.ngOnInit();
  ...
  Search(op: number) {
    if (op == 1) {
      this.ingresosFilter = this.ingresos.filter((ingreso: { fecha: string }) => {
        let filter = true;
        if (this.search) {
          filter = ingreso.fecha.toLowerCase().includes(this.search.toLowerCase());
        }
        return filter;
      });
    } else {
  ...
  countRangeRegister(): string {
    ...
    const endIndex = Math.min(startIndex + this.recordPerPage - 1, this.ingresosFilter.length);
    let msg;
  ...
  public deleteProducto(index: number, idProducto?: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (idProducto) {
          this.IngresosService.deleteProducto(idProducto).subscribe(
            response => {
              if (response.data) {
                this.toastr.success(response.message, '¡Listo!', {closeButton: true});
              }
